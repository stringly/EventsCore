    ...
    public static class HttpClientExtensions
    {
            ...
            IEnumerable<KeyValuePair<string, string>> formValues)
        {
            foreach (var kvp in formValues)
            {
                ...
                {
                    var element = (IHtmlTextAreaElement)form[kvp.Key];
                    element.Value = kvp.Value;
                }
                else if (kvp.Key == "EventSeriesId" || kvp.Key == "EventTypeId" || kvp.Key == "State")
                {
                    var element = (IHtmlSelectElement)form[kvp.Key];
                    element.Value = kvp.Value;
                }
                else
                {
